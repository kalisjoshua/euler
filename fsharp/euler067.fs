...
let rec goDeep tree depth= 
    match depth with
    | 0 ->
        match tree with 
        | Node(x,l,r) -> [(x + (Value l)); (x + (Value r))] |> List.max
        | Leaf(x) -> x
    | _ ->
        match tree with 
        | Node(x,l,r) -> [(x + (goDeep l (depth-1))); (x + (goDeep r (depth-1)))] |> List.max
        | Leaf(x) -> x

let pickDirection tree=
    match tree with
    | Node(x,l,r) -> if (goDeep l 15) > (goDeep r 15) then l else r
    | Leaf(x) -> failwith "cant do it"

let pTree = parseTree tri 0 []
let rec traverse tree total=
    match tree with
    | Node(x,l,r) -> traverse  (tree |> pickDirection) (total + x)
    | Leaf(x) -> (total+x)
let result = traverse pTree 0 |> Dump
